...
{
    public class UpdateHocSinhCommandValidator : AbstractValidator<UpdateHocSinhCommand>
    {
        private readonly IApplicationDbContext _context;

        public UpdateHocSinhCommandValidator(IApplicationDbContext context)
        {
            _context = context;

            RuleFor(x => x.Id)
                .NotEmpty().WithMessage("ID Học sinh không được để trống.")
                .MustAsync(BeExistingHocSinhId).WithMessage("Học sinh không tìm thấy.");
        ...
        private async Task<bool> BeExistingHocSinhId(Guid id, CancellationToken cancellationToken)
        {
            return await _context.HocSinhs.AnyAsync(h => h.Id == id, cancellationToken);
        }
